...
ulimit -n 4096
OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 deppol --ztfname=$p -j 5 --wd=${2} --lc-folder=\$SCENEMODELING_FOLDER/lc --func=make_catalog,mkcat2,makepsf,match_gaia,filter_seeing,filter_psfstars_count,reference_quadrant,astrometry_fit,astrometry_fit_plot,photometry_fit,photometry_fit_plot,smphot --lc-folder=/sps/ztf/data/storage/scenemodeling/lc --quadrant-workspace=/dev/shm/llacroix --rm-intermediates --scratch=/tmp/llacroix --astro-degree=5 --max-seeing=4. --discard-calibrated --astro-min-mag=-10."
